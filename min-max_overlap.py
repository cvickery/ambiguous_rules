...
def rule_facts():
  return [0, 0, 0, 0]  # Num cases, Num Src, NumDst, Num Dst is blanket
...
select d.course_id, d.discipline, d.catalog_number, c.course_status, c.attributes
  ...
  where rule_id = {rule_id}
    and d.course_id = c.course_id
                        """)
  rule_info[rule_key][2] += format_cursor.rowcount
  courses = []
...
rule_info = defaultdict(rule_facts)

if __name__ == '__main__':

  rule_cursor.execute("""
select s1.course_id, s1.discipline, s1.catalog_number, c.course_status,
        s1.min_gpa as min_1, s1.max_gpa as max_1,
        r1.id as id_1, rule_key(r1.id) as key_1, r1.priority as priority_1,
        s2.min_gpa as min_2, s2.max_gpa as max_2,
        r2.id as id_2, rule_key(r2.id) as key_2, r2.priority as priority_2
  ...
  order by course_id
                 """)
  with open('./min_max_overlap.csv', 'w') as mmo_file:
    print('course_id,course,min_1,max_1,rule 1,min_2,max_2,rule 2', file=mmo_file)
    # For any one course, no rule should be seen more than once
    ...
    for row in rule_cursor.fetchall():
      if row.priority_1 == row.priority_2 and row.max_1 > row.min_2 or row.max_2 > row.min_1:
        if row.id_1 in course_rules[row.course_id] and row.id_2 in course_rules[row.course_id]:
          continue
        course_rules[row.course_id].add(row.id_1)
        course_rules[row.course_id].add(row.id_2)
        text_1 = format_rule(row.id_1)
        text_2 = format_rule(row.id_2)
        print(f'[{row.course_id:06} {row.course_status}], {row.discipline} {row.catalog_number},'
