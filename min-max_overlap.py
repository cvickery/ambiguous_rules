...
conn = PgConnection()
first_cursor = conn.cursor()  # Used to find potentially ambiguous rules
...
-- Look up potential ambiguities: cases where the only way to tell which rule applies would be a
-- difference in grade requirements.
...
""")
      ...
      # How do the sets of receiving courses compare? (We know the sending course sets and the rule
      ...
      dest_1 = set([(rule.course_id, rule.offer_nbr) for rule in rules_info[0]['dest_courses']])
      dest_2 = set([(rule.course_id, rule.offer_nbr) for rule in rules_info[1]['dest_courses']])
